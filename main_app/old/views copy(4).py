...
def test(request):
  ...
  #  url = "https://services.swpc.noaa.gov/text/aurora-nowcast-map.txt"
    ...
    n_contours = 120#contour_data['category'].nunique()#40
    levels = np.linspace(start=3, stop=100, num=n_contours)
   ...
   # return HttpResponse(arr1[10000][0])
'''
    url = "https://services.swpc.noaa.gov/text/aurora-nowcast-map.txt"
    #return HttpResponse(str(x[0].decode()));
    #if(str(x[0].decode())=='none'):
...
#    for line in fff.decode().split('x'):
        if (ctr == 1025):
            ctr = 0
            ct += 1
        if '#' in line.decode():
